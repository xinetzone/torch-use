...
class CV:
    ...
    @staticmethod
    def train(net, device, train_iter, test_iter,
        ...
        logger.info(f'loss {metric[0] / metric[2]:.3f}, train acc '
              f'{metric[1] / metric[3]:.3f}, test acc {test_acc:.3f}')
        logger.info(f'{metric[2] * num_epochs / timer.sum():.1f} examples/sec on '
              f'{str(device)}')

    @staticmethod
    def train_fine_tuning(net, device,
        ...
        if param_group:
            ...
            trainer = torch.optim.SGD(net.parameters(), lr=learning_rate,
                                      weight_decay=0.001)
        net.train()
        CV.train(net, device, train_iter, test_iter,
                 loss, trainer, num_epochs,
                 ylim=ylim,
